# Companion patch — drop-in Kotlin + wiring for the Ellie connection
...
- **`EllieClient.kt`** → `app/src/main/kotlin/com/dominionlabs/elicompanion/network/EllieClient.kt`
...
1. **`data/SettingsManager.kt`** — add one setting:
   ```kotlin
   var ellieAuthToken: String?    // the ELLI_AUTH_TOKEN from ~/.elli_env on device
       get() = prefs.getString("ellie_auth_token", null)
       set(v) { prefs.edit().putString("ellie_auth_token", v).apply() }
   ```
...
2. **`network/mTLSClient.kt`** — tee captured events to the on-device store.
   ...
   ```kotlin
   // Mirror to on-device Ellie so she can use this context (fire-and-forget).
   EllieClient(settingsManager).teeContextEvent(type, payloadAsJsonObject)
   ```
...
4. **Device side (`eli/runtime/device_inference/tool_router.py`)** — 3 lines in
   `main()` so Ellie gets the context tools + grounding:
   ```python
   import ellie_router_ext as ext          # from this folder, deployed alongside
...
- `connector/AppConnector.kt` — Ellie's tool loop → `EliAgenticEngine` (item 7).
